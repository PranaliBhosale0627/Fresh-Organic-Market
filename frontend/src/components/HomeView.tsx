...
export default function HomeView({
  ...
  return (
    <div className="px-3 sm:px-4 md:px-10 lg:px-12 py-4 sm:py-6 space-y-8 sm:space-y-10 max-w-7xl mx-auto">
      {/* Search Input Section */}
      <section className="relative max-w-2xl mx-auto mt-2 sm:mt-4 animate-rise-in">
        <form onSubmit={handleSearchSubmit} className="relative group">
          <input
            ...
            className="glass-panel w-full h-14 sm:h-16 pl-12 pr-24 rounded-full border border-white/70 bg-white shadow-[0_10px_35px_rgba(45,90,39,0.08)] focus:ring-2 focus:ring-primary/20 text-sm transition-all focus:shadow-[0_18px_45px_rgba(45,90,39,0.16)] outline-none"
          />
          ...
          <button 
            type="submit"
            className="absolute right-3 top-1/2 -translate-y-1/2 bg-primary text-white text-xs font-bold px-5 py-2.5 rounded-full hover:bg-primary-container transition-all active:scale-95 shadow-lg shadow-primary/15"
          >
            Find
          </button>
        </form>
      ...
      <section className="animate-rise-in stagger-1">
        <div className="relative h-[360px] sm:h-[390px] md:h-[440px] rounded-[28px] sm:rounded-[36px] overflow-hidden group shadow-xl shadow-primary/10 border border-outline-variant/20 interactive-lift">
          ...
          ></div>
          <div className="absolute inset-0 z-20 flex flex-col justify-center px-6 sm:px-8 md:px-16 max-w-2xl text-left">
            ...
            <h2 className="text-white font-display-lg text-4xl sm:text-5xl md:text-6xl leading-tight mb-3 max-w-[11ch]">
            ...
            <button 
              ...
              className="bg-primary text-on-primary px-8 py-3.5 rounded-full font-bold text-sm w-fit hover:bg-primary-container transition-all hover:shadow-xl hover:shadow-primary/30 active:scale-95"
            >
              Shop Seasonal Picks
            </button>
          </div>
        ...
        <div className="flex overflow-x-auto gap-4 pb-2 scrollbar-none scroll-smooth">
          {categories.map((cat, index) => (
            <button
              ...
              className="flex-shrink-0 w-24 sm:w-28 group cursor-pointer text-center focus:outline-none animate-rise-in"
              style={{ animationDelay: `${120 + index * 45}ms` }}
            >
              <div className={`w-20 h-20 sm:w-24 sm:h-24 rounded-full ${cat.colorBg} flex items-center justify-center mb-2 mx-auto interactive-lift shadow-sm border border-outline-variant/10 group-hover:shadow-lg`}>
                <span className="text-3xl sm:text-4xl transition-transform duration-300 group-hover:rotate-6 group-hover:scale-110">{cat.icon}</span>
              </div>
      ...
      <section className="space-y-6 animate-rise-in stagger-3">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 sm:gap-5 lg:gap-6">
          {trendingProducts.map((p, index) => {
            const isFav = wishlistItems.some(item => item.id === p.id);
            const isAdded = addedItemIds.includes(p.id);
            return (
              <div
                key={p.id}
                onClick={() => onSelectProduct(p.id)}
                className="bg-white rounded-[22px] sm:rounded-[26px] overflow-hidden shadow-sm hover:shadow-xl hover:shadow-primary/10 interactive-lift group border border-outline-variant/30 flex flex-col h-full cursor-pointer relative animate-rise-in"
              ...
              >
                ...
                {/* Details Section */}
                <div className="p-3 sm:p-4 flex flex-col flex-grow text-left justify-between">
                  <div>
      ...
      <section className="pb-10 animate-rise-in stagger-4">
        <div className="bg-primary-container text-on-primary-container rounded-[28px] sm:rounded-[36px] p-6 md:p-12 flex flex-col md:flex-row items-center gap-8 relative overflow-hidden shadow-xl shadow-primary/10">
          ...
          <div className="relative z-10 w-full md:w-1/3 max-w-[280px] aspect-square rounded-2xl overflow-hidden shadow-2xl border-4 border-white/10 animate-soft-float">
            <img
