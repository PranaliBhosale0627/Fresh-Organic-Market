...
export default function CartView({
  ...
  return (
    <div className="px-3 sm:px-4 md:px-10 lg:px-12 py-5 sm:py-8 max-w-7xl mx-auto text-left space-y-6 sm:space-y-8 animate-rise-in">
      {/* View Header */}
      <div className="flex items-center justify-between border-b border-outline-variant/10 pb-4 animate-rise-in">
      ...
      {cartItems.length === 0 ? (
        /* Empty Cart State */
        <div className="py-16 sm:py-20 text-center space-y-6 max-w-md mx-auto bg-white rounded-3xl border border-outline-variant/30 p-8 shadow-xl shadow-primary/10 animate-rise-in">
          <div className="w-20 h-20 bg-surface-container rounded-full flex items-center justify-center mx-auto text-on-surface-variant/40 animate-soft-float">
          ...
          <button
            ...
            className="bg-primary text-on-primary px-8 py-3.5 rounded-full font-bold text-sm shadow-md hover:bg-primary-container hover:shadow-xl hover:shadow-primary/25 transition-all inline-block active:scale-95"
          >
            Start Shopping
          </button>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-8 items-start">
          ...
          <div className="lg:col-span-8 space-y-4">
            {cartItems.map((item, index) => (
              <div
                key={item.product.id}
                className="bg-white rounded-2xl p-4 flex flex-col sm:flex-row items-center gap-4 border border-outline-variant/30 shadow-sm hover:shadow-xl hover:shadow-primary/10 interactive-lift animate-rise-in"
                style={{ animationDelay: `${Math.min(index, 6) * 55}ms` }}
              >
                {/* Product Image */}
                <div className="w-full sm:w-24 h-40 sm:h-24 rounded-xl overflow-hidden flex-shrink-0 bg-surface-container-low border border-outline-variant/10">
                  <img
          ...
          <div className="lg:col-span-4 space-y-6 lg:sticky lg:top-28 animate-rise-in stagger-2">
            {/* Coupon Code section */}
            <div className="bg-white rounded-2xl p-6 border border-outline-variant/30 shadow-sm hover:shadow-lg transition-shadow space-y-3">
            ...
            {/* Total Pricing panel */}
            <div className="bg-primary-container text-on-primary-container rounded-2xl p-6 md:p-8 shadow-xl shadow-primary/15 space-y-6 border border-primary/20">
              ...
              <div className="space-y-3 pt-2">
                <button
                  ...
                  className="w-full bg-white text-primary-container py-4 rounded-full font-bold text-sm shadow-xl hover:bg-opacity-95 hover:-translate-y-0.5 transition-all active:scale-[0.98] flex items-center justify-center gap-2 group border border-white/10"
                >
                  Proceed to Checkout
            ...
            {/* Delivery Eco Notice */}
            <div className="flex items-start gap-3 p-4 bg-tertiary-fixed text-on-tertiary-fixed rounded-2xl border border-tertiary-fixed-dim/20 shadow-sm interactive-lift">
